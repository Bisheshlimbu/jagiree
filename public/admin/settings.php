...
$allowedTabs = ['general', 'notifications', 'integration', 'cron'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($section === 'notifications') {
        ...
        if ($result['success']) {
            flashSet('success', $result['message']);
            header('Location: /admin/settings.php?tab=notifications');
            exit;
        }
    ...
    if ($section === 'cron') {
        $result = updateApifyScheduleSettings([
            'apify_schedule_enabled' => !empty($_POST['apify_schedule_enabled']),
            'apify_schedule_interval_hours' => (int) ($_POST['apify_schedule_interval_hours'] ?? 24),
        ]);

        if ($result['success']) {
            flashSet('success', $result['message']);
            header('Location: /admin/settings.php?tab=cron');
            exit;
        }

        $errorCron = $result['error'];
        $activeTab = 'cron';
        $settings['apify_schedule_enabled'] = !empty($_POST['apify_schedule_enabled']) ? '1' : '0';
        $settings['apify_schedule_interval_hours'] = (string) (int) ($_POST['apify_schedule_interval_hours'] ?? 24);
    }
...
$apifyScheduleEnabled = ($settings['apify_schedule_enabled'] ?? '0') === '1';
$apifyScheduleHours = (int) ($settings['apify_schedule_interval_hours'] ?? 24);
$apifyNextSyncAt = apifyNextScheduledSyncAt();
$apifyScheduleLastAttempt = trim($settings['apify_schedule_last_attempt_at'] ?? '');
$apifyScheduleLastResult = trim($settings['apify_schedule_last_result'] ?? '');
$apifyCronCommand = 'php ' . escapeshellarg(dirname(__DIR__, 2) . '/scripts/sync-linkedin-jobs.php');
$apifyReady = apifyIntegrationConfigured() && siteSettingEnabled('apify_enabled');
...
<nav class="settings-tabs" aria-label="Settings sections">
    <a href="?tab=general" class="settings-tabs__link<?= $activeTab === 'general' ? ' is-active' : '' ?>">General</a>
    <a href="?tab=notifications" class="settings-tabs__link<?= $activeTab === 'notifications' ? ' is-active' : '' ?>">Notifications</a>
    <a href="?tab=integration" class="settings-tabs__link<?= $activeTab === 'integration' ? ' is-active' : '' ?>">Integration</a>
    <a href="?tab=cron" class="settings-tabs__link<?= $activeTab === 'cron' ? ' is-active' : '' ?>">Cron</a>
</nav>
...
<section class="panel panel--integration">
        ...
        <div class="integration-provider">
            <div class="integration-provider__head">
                <div>
                    ...
                    <p>Store credentials here. Configure scheduled sync under the Cron tab.</p>
                </div>
            ...
            <div class="integration-sync-fields">
                <label class="form-field">
                    <span>Default search keywords (optional)</span>
                    <input type="text" name="apify_job_keywords" value="<?= htmlspecialchars($settings['apify_job_keywords']) ?>" maxlength="200" placeholder="e.g. Frontend developer, SEO">
                    <p class="form-hint">Fallback when search URL is empty. Need at least keywords or a search URL to sync.</p>
                </label>

                <label class="form-field">
                    <span>Default location (optional)</span>
                    <input type="text" name="apify_job_location" value="<?= htmlspecialchars($settings['apify_job_location']) ?>" maxlength="120" placeholder="e.g. Nepal">
                </label>
    ...
    <form class="integration-test-form" method="post" action="/admin/api/sync-linkedin-jobs.php" id="apifySyncForm">
        <button type="submit" class="btn-sm btn-sm--primary" id="apifySyncBtn" <?= $apifyReady ? '' : 'disabled' ?>>Sync LinkedIn jobs now</button>
    </form>
...
<section class="panel panel--integration">
    <div class="panel-header panel-header--compact">
        <h2>Cron</h2>
        <p>Schedule automatic LinkedIn job syncs. Requires Apify to be enabled under Integration.</p>
    </div>

    <?php if ($errorCron): ?>
        <div class="admin-flash admin-flash--error settings-flash" role="alert"><?= htmlspecialchars($errorCron) ?></div>
    <?php endif; ?>

    <?php if (!$apifyReady): ?>
        <div class="admin-flash admin-flash--error settings-flash" role="status">
            Enable and save Apify credentials on the <a href="?tab=integration">Integration</a> tab before configuring cron.
        </div>
    <?php endif; ?>
        ...
        <div class="integration-provider">
            <label class="toggle-field">
                <span class="toggle-field__text">
                    ...
                    <small>Automatically pull LinkedIn jobs on an interval (system cron must call the script below).</small>
                </span>
                <input type="checkbox" name="apify_schedule_enabled" value="1" class="toggle-field__input" <?= $apifyScheduleEnabled ? 'checked' : '' ?> <?= $apifyReady ? '' : 'disabled' ?>>
                <span class="toggle-switch" aria-hidden="true"></span>
            </label>

            <label class="form-field">
                <span>Sync frequency</span>
                <select name="apify_schedule_interval_hours" <?= $apifyReady ? '' : 'disabled' ?>>
                    <?php foreach (apifyScheduleIntervalOptions() as $hours => $label): ?>
                    <option value="<?= (int) $hours ?>" <?= $apifyScheduleHours === (int) $hours ? 'selected' : '' ?>>
            ...
            <div class="integration-schedule-box">
                <?php if ($apifyScheduleEnabled && $apifyReady): ?>
                <p class="integration-meta integration-meta--success">
                    Schedule on · next due
                    <?= $apifyNextSyncAt ? htmlspecialchars(date('Y-m-d H:i', $apifyNextSyncAt)) : 'now' ?>
                    <?= apifyScheduledSyncIsDue() ? ' (due now)' : '' ?>
                </p>
                <?php else: ?>
                <p class="integration-meta">Schedule is off. Enable integration, turn on scheduled sync, then add the cron job below.</p>
                <?php endif; ?>
                <?php if ($apifyScheduleLastAttempt !== ''): ?>
                <p class="integration-meta">
                    Last scheduled attempt: <?= htmlspecialchars($apifyScheduleLastAttempt) ?>
                    <?php if ($apifyScheduleLastResult !== ''): ?>
                    — <?= htmlspecialchars($apifyScheduleLastResult) ?>
                    <?php endif; ?>
                </p>
                <?php endif; ?>
                <p class="form-hint">Add this cron entry so the schedule can run (every 15 minutes is enough; the script only syncs when due):</p>
                <pre class="integration-cron-code"><?= htmlspecialchars('*/15 * * * * ' . $apifyCronCommand) ?></pre>
            </div>
        </div>

        <div class="integration-actions">
            <button type="submit" class="btn-sm btn-sm--primary" <?= $apifyReady ? '' : 'disabled' ?>>Save cron schedule</button>
        </div>
    </form>
</section>
<?php endif; ?>
