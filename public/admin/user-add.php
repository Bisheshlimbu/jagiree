...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $form = [
        ...
        'role' => $_POST['role'] ?? ROLE_SEEKER,
        'company_name' => trim($_POST['company_name'] ?? ''),
        'status' => $_POST['status'] ?? 'verified',
        'password' => $_POST['password'] ?? '',
    ];

    $result = createUserByAdmin(array_merge($form, [
...
$avatarUrl = userAvatarPlaceholderUrl();

$pageTitle = 'Add User — Jagiree Admin';
$activePage = 'users';
...
<section class="panel panel--form">
    <div class="panel-header panel-header--compact">
        <div>
            <h2>Add User</h2>
            <p>Create a new seeker or employer account.</p>
        </div>
    ...
    <form class="settings-form settings-form--wide" method="post" enctype="multipart/form-data" novalidate>
        <div class="avatar-field">
            ...
            <div class="avatar-field__row">
                <img src="<?= htmlspecialchars($avatarUrl) ?>" alt="" class="avatar-field__preview user-avatar--placeholder" id="avatarPreview">
                <div class="avatar-field__controls">
                    ...
                    <p class="avatar-field__hint">Optional. JPG, PNG, or WEBP. Max 2MB.</p>
                </div>
            </div>
        </div>
        ...
        <label class="form-field">
            <span>Full name *</span>
            <input type="text" name="full_name" value="<?= htmlspecialchars($form['full_name']) ?>" required autofocus>
        </label>
        ...
        <label class="form-field">
            ...
            <input type="text" name="username" value="<?= htmlspecialchars($form['username']) ?>" pattern="[a-zA-Z0-9_]+" minlength="3">
            <small class="form-field__hint">If left blank, one is created from the email.</small>
        </label>
        ...
        <div class="form-actions">
            <button type="submit" class="btn-sm btn-sm--primary">Create User</button>
